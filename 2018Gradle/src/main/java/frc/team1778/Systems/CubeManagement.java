...
public class CubeManagement {
  ...
  public static void initialize() {
    ...
    lowerLiftMotor =
        configureMotor(
            HardwareIDs.LOWER_LIFT_TALON_ID, LOWER_REVERSE_MOTOR, HardwareIDs.UPPER_LIFT_TALON_ID);
  ...
  public static void resetMotors() {
    ...
    leftCollectorMotor.set(
        COLLECTOR_IN_AUTO_STRENGTH); // low-level default collection (retention only)
  ...
  public static void liftBrakeOn() {
    ...
    InputOutputComm.putBoolean(
        InputOutputComm.LogTable.kMainLog, "CubeMgmt/LiftBrake", liftBrakeOn);
  ...
  public static void collectCube(double strength) {
    InputOutputComm.putDouble(
        InputOutputComm.LogTable.kMainLog, "CubeMgmt/CollectorStrength", strength);
  ...
  public static void runLift(double liftStrength) {
    InputOutputComm.putDouble(
        InputOutputComm.LogTable.kMainLog, "CubeMgmt/LiftStrength", liftStrength);
  ...
  private static void checkCollectorControls() {
    ...
    if (collectorInStrength > 0) collectorMotorStrength = collectorInStrength * COLLECTOR_IN_FACTOR;
      ...
      collectorMotorStrength =
          COLLECTOR_IN_AUTO_STRENGTH; // default auto motor rate in (for cube retention)

    InputOutputComm.putDouble(
        InputOutputComm.LogTable.kMainLog, "CubeMgmt/CollectorStrength", collectorMotorStrength);
  ...
  private static void checkLiftControls() {
    ...
    if (Math.abs(liftStrength) >= LIFT_MOTOR_DEAD_ZONE)
      if (liftStrength < 0) liftStrength *= LIFT_MOTOR_UP_FACTOR;
      else liftStrength *= LIFT_MOTOR_DOWN_FACTOR;
    else liftStrength = 0.0;
