...
public class DriveAssembly {
  ...
  public static void initialize() {
    ...
    mFrontLeft =
        configureMotor(
            HardwareIDs.LEFT_FRONT_TALON_ID,
            LEFT_REVERSE_MOTOR,
            ALIGNED_LEFT_SENSOR,
            kP,
            kI,
    ...
    mFrontRight =
        configureMotor(
            HardwareIDs.RIGHT_FRONT_TALON_ID,
            RIGHT_REVERSE_MOTOR,
            ALIGNED_RIGHT_SENSOR,
            kP,
            kI,
            kD,
            kF);
    mBackLeft =
        configureMotor(
            HardwareIDs.LEFT_REAR_TALON_ID, LEFT_REVERSE_MOTOR, HardwareIDs.LEFT_FRONT_TALON_ID);
    mBackRight =
        configureMotor(
            HardwareIDs.RIGHT_REAR_TALON_ID, RIGHT_REVERSE_MOTOR, HardwareIDs.RIGHT_FRONT_TALON_ID);
  ...
  public static double getDistanceInches() {
    ...
    double rightPos =
        mFrontRight.getSelectedSensorPosition(0) * HardwareIDs.INCHES_PER_ENCODER_PULSE;
  ...
  public static void autoMagicTurn(
    ...
    mFrontLeft.configMotionAcceleration(accelNativeUnits, TIMEOUT_MS);
    mFrontLeft.set(
        ControlMode.MotionMagic, targetPosInchesLeft / HardwareIDs.INCHES_PER_ENCODER_PULSE);

    // right front drive straight - uses motion magic
    mFrontRight.configMotionCruiseVelocity(nativeUnitsPer100ms, TIMEOUT_MS);
    mFrontRight.configMotionAcceleration(accelNativeUnits, TIMEOUT_MS);
    mFrontRight.set(
        ControlMode.MotionMagic, targetPosInchesRight / HardwareIDs.INCHES_PER_ENCODER_PULSE);
  ...
  public static void drive(double leftValue, double rightValue) {
    ...
    // String myString2 = new String("leftSpeed = " + leftSpeedStr + " rightSpeed = " +
    // rightSpeedStr);
